...
-export([add/1, lookup/1, update_cart/3, get_cart/1, sum_cart/1, update_bill_address/2,
	...
	add_transaction/2, get_user_name/1]).
-export([sum/2, write/3, delete/2, check_address/1]).

%% Basic functionality for opening and closing the database

% Creates a new in memory table and opens the database on the disk
% - Input: FileName
% - Output: ok
new(FileName) ->
	% In memory database
	ets:new(refTableRam, [named_table, public, set]),
	% Database on the disk
	dets:open_file(refTableDisk,[{file,FileName}]),
	ok.
